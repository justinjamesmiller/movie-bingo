    ...
    <div className="modal">
      <div className="modal-content">
        <h3>
          ...
          {isReplace ? ' wants to swap out this trope:' : isUndo ? ' wants to undo this space:' : ' claims this happened:'}
        </h3>
        ...
        {!isClaimant && !hasVoted && (
          <div className="claim-vote-buttons">
            <button className="btn agree" onClick={onAgree}>
              {isReplace ? '👍 Agree, swap it out' : isUndo ? '👍 Agree, undo it' : '👍 Agree, it happened'}
            </button>
            <button className="btn disagree" onClick={onDisagree}>
              {isReplace ? '👎 Keep it as is' : isUndo ? '👎 Keep it marked' : '👎 Disagree'}
            </button>
          </div>
        )}
        {(isClaimant || hasVoted) && (
